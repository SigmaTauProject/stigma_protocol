...
class TerminalNetworkMaster {
	this() {
		void handleWebSocketConnection(scope WebSocket socket) {
			...
			_newTerminals ~= new TerminalNetwork(socket);
		...
		void main() {
			this.newTerminals = NewTerminals(&this._newTerminals);
	...
	TerminalNetwork[] _newTerminals = [];
	NewTerminals newTerminals;
	
	private struct NewTerminals {
		TerminalNetwork[]* _newTerminals;
		@property bool empty() {
			return _newTerminals.length==0;
		}
		@property TerminalNetwork front() {
			assert(!empty);
			return (*_newTerminals)[0];
		}
		void popFront() {
			assert(!empty);
			*_newTerminals = (*_newTerminals)[1..$];
		}
	}
	auto getNewTerminals() {
		auto toReturn = _newTerminals;
		_newTerminals = [];
		return toReturn;
	}
}
